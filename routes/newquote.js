  ...
  asyncHandler(async function (req, res, next) {
    ...
    if (response.status === 400) {
      ...
      if (
        !data.some(
          (element) =>
            element.number.toLowerCase() === req.body.houseNumber.toLowerCase()
        )
      ) {
        res
          .status(response.status)
          .json({ Message: "House number is incorrect" });
      }
      if (
        !data.some(
          (element) =>
            element.street.toLowerCase() === req.body.streetName.toLowerCase()
        )
      ) {
        res
          .status(response.status)
          .json({ Message: "Street name is incorrect" });
      }
      if (
        !data.some(
          (element) =>
            element.posttown.toLowerCase() === req.body.city.toLowerCase()
        )
      ) {
    ...
    const finalData = {
      ...
      city: req.body.city,
      polictyStart: req.body.polictyStartDate,
      policyDate: new Date(),
      quotePrice: totalPrice,
      referenceNumber: refNumber,
    };

    res.json({ quote: finalData });
  })
);
